...
static void cpp_merge_rows( Mat* dest, int * c, int * out, int rows, int cols  )
{
  uint8_t __restrict * b = dest->data;
  int * o_p = out;
  int * c_p = c  ;
  uint8_t __restrict * next_b = b + cols ;
  int * next_c = c + cols ;-----------------------------------
...
int main(void)
{
    ...
    cpp_merge_rows( &dest, &class_seg[0], &classr_seg[0], dest.rows, dest.cols );
    waitKey(0);
    return 0;
}
