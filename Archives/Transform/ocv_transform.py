...
def setState(element,state):
    ...
    if element.name == 'btn':
        ...
        if state == 2:
            ...
            cv2.putText(element.img,'THRESHOLD',(5,27), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,0,255),2)
            cv2.ellipse(element.img,(0,element.h),(element.w/2,element.h/2),0,270,360,(255,0,0),-1)
            cv2.putText(element.img,'GRADIENT',(5,135), cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,0),2)
            cv2.ellipse(element.img,(element.w,0),(element.w/2,element.h/2),0,90,180,(0,255,0),-1)
            sz,bl = cv2.getTextSize('EDGES',cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
            cv2.putText(element.img,'EDGES',(495-sz[0],27), cv2.FONT_HERSHEY_SIMPLEX, 1,(255,0,0),2)
            cv2.ellipse(element.img,(element.w,element.h),(element.w/2,element.h/2),0,180,270,(0,0,255),-1)
            sz,bl = cv2.getTextSize('CONTOUR',cv2.FONT_HERSHEY_SIMPLEX, 1, 2)
            cv2.putText(element.img,'CONTOUR',(495-sz[0],135), cv2.FONT_HERSHEY_SIMPLEX, 1,(255,255,255),2)
        ...
        if btn.inR(x,y):
            ...
            if btn.state == 2:
                ...
                if (cB,cG,cR) == (255,255,255):
                    ...
                    imgt3 = cv2.adaptiveThreshold(imgt2,255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,11,2)
                    outx.img = cv2.cvtColor(imgt3, cv2.COLOR_GRAY2BGR)
                    return
                if (cB,cG,cR) == (0,0,255):
                    print 'the red rites'
                    imgt1 = np.zeros((op.h,op.w,3), np.uint8)
                    imgt1[:,:,:] = op.img[:,:,:]
                    imgray = cv2.cvtColor(imgt1,cv2.COLOR_BGR2GRAY)
                    ret,thresh = cv2.threshold(imgray,127,255,0)
                    _,contours,_ = cv2.findContours(thresh,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
                    cv2.drawContours(imgt1, contours, -1, (0,0,255), 3)
                    outx.img = imgt1
                    return
                if (cB,cG,cR) == (255,0,0):
                    ...
                    imgt1 = op.img
                    imgt2 = cv2.cvtColor(imgt1, cv2.COLOR_BGR2GRAY)
                    imgt3 = cv2.Laplacian(imgt2,cv2.CV_16U)
                    outx.img = cv2.cvtColor(imgt3, cv2.COLOR_GRAY2BGR)
                    return
                if (cB,cG,cR) == (0,255,0):
